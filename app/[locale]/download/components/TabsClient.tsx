...
const TabsClient = ({ architectures, translations }: TabsClientProps) => {
  ...
  const currentArch = availableArchitectures.includes(archFromUrl || "")
    ? archFromUrl || availableArchitectures[0] || "x86_64"
    : availableArchitectures[0] || "x86_64";
  ...
  // Set default architecture on initial load
  useEffect(() => {
    if (
      ...
      const defaultArch = availableArchitectures[0];
      const params = new URLSearchParams(searchParams.toString());
      params.set("arch", defaultArch);
      router.replace(`${pathname}?${params.toString()}`, { scroll: false });
      isInitialLoad.current = false;
    }
  }, [archFromUrl, availableArchitectures, pathname, searchParams, router]);
